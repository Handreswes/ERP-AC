...
foreach ($file in $files) {
    ...
    Write-Host "Processing: $githubPath"
    ...
    $body = @{
        message = "Final Sync V68: $githubPath"
    ...
    try {
        ...
        Write-Host "Successfully synced: $githubPath"
    }
    catch {
        Write-Host "Failed to sync $($githubPath): $($_.Exception.Message)" -ForegroundColor Red
    }
}

Write-Host "`nSincronización finalizada." -ForegroundColor Cyan
